...
struct DockContext
{
	...
	{
		int dc = m_docks.size();
		fwrite(&dc, sizeof(int), 1, file);
		for (int i = 0; i < dc; i++)
		{
			Dock& dock = *m_docks[i];
			unsigned long long ptr = (unsigned long long)&dock;
			//fwrite(&ptr, sizeof(unsigned long long), 1, file);
			fwrite(&i, sizeof(int), 1, file);
			int labellen = strlen(dock.label) + 1; // include null terminator
			fwrite(&labellen, sizeof(int), 1, file);
			fwrite(dock.label, sizeof(char), strlen(dock.label) + 1, file);
			int xpos = (int)dock.pos.x;
			int ypos = (int)dock.pos.y;
			fwrite(&xpos, sizeof(int), 1, file);
			fwrite(&ypos, sizeof(int), 1, file);
			fwrite(dock.location, sizeof(char), 16, file);

#define FWRITE_INT(f, intv) fwrite(&intv, sizeof(int), 1, f); 
#define FWRITE_CHAR(f, chrv) fwrite(&chrv, sizeof(char), 1, f); 

			int xsize = (int)dock.size.x;
			int ysize = (int)dock.size.y;
			char status = (char)dock.status;
			char active = (char)dock.active;
			char opened = (char)dock.opened;
			int prev = (int)getDockIndex(dock.prev_tab);
			int next = (int)getDockIndex(dock.next_tab);
			int child0 = (int)getDockIndex(dock.children[0]);
			int child1 = (int)getDockIndex(dock.children[1]);
			int parent = (int)getDockIndex(dock.parent);
			FWRITE_INT(file, xsize);
			FWRITE_INT(file, ysize);
			FWRITE_CHAR(file, status);
			FWRITE_CHAR(file, active);
			FWRITE_CHAR(file, opened);
			FWRITE_INT(file, prev);
			FWRITE_INT(file, next);
			FWRITE_INT(file, child0);
			FWRITE_INT(file, child1);
			FWRITE_INT(file, parent);
		}
		/*
		file << "docks = {\n";
		for (int i = 0; i < m_docks.size(); ++i)
...
#define READ_INT(intr)		fread(&intr, sizeof(int), 1, file);
#define READ_CHAR(chrr)		fread(&chrr, sizeof(char), 1, file);
#define READ_ULL(ullr)		fread(&ullr, sizeof(unsigned long long), 1, file);
		...
		for (int i = 0; i < dockCount; i++)
		{
			...
			//READ_ULL(ptr); // it's not actually used, gg
			READ_INT(indx);
			Dock& d = *m_docks[indx];
			d.last_frame = 0;
			d.invalid_frames = 0;
			...
			READ_INT(labellen);
			char label[256];
			memset(label, 0, 256*sizeof(char));
			fread(label, sizeof(char), labellen, file);
			d.label = ImStrdup(label);
			...
			d.pos.x = xpos;
			d.pos.y = ypos;
			fread(d.location, sizeof(char), 16, file);
			int xsize = 0; READ_INT(xsize); d.size.x = xsize;
			int ysize = 0; READ_INT(ysize); d.size.y = ysize;
			char status = 0; READ_CHAR(status); d.status = (Status_)status;
			char active = 0; READ_CHAR(active); d.active = active;
			char opened = 0; READ_CHAR(opened); d.opened = opened;
			int prev = 0; READ_INT(prev); d.prev_tab = getDockByIndex(prev);
			int next = 0; READ_INT(next); d.next_tab = getDockByIndex(next);
			int child0 = 0; READ_INT(child0); d.children[0] = getDockByIndex(child0);
			int child1 = 0; READ_INT(child1); d.children[1] = getDockByIndex(child1);
			int parent = 0; READ_INT(parent); d.parent = getDockByIndex(parent);
...
void SaveDock(FILE * file)
{
	g_dock.save(file);
}


void LoadDock(FILE * file)
{
	g_dock.load(file);
}
